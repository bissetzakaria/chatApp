...
const API_KEY = "AIzaSyCRHqZRsEhESroMg5t0gdMMhcAM3qYsMSU";
 ...
 */
const auth = (url, email, password, nom, tel, callback) => {
...
const auth1 = (url, email, password, callback) => {
	const resquestUrl = `https://identitytoolkit.googleapis.com/v1/${url}?key=${API_KEY}`;
	const requestBody = { email, password, returnSecureToken: true };
 ...
 */
export const signUp = (email, password, nom, tel, callback) => {
	auth("accounts:signUp", email, password, nom, tel, callback);
};
...
export const login = (email, password, callback) => {
	auth1("accounts:signInWithPassword", email, password, callback);
};
